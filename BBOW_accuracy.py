...
def method_Log_regression(x_train,x_test):
    ...
    accu = accu*100
    print("Accuracy on BBOW with Logistic regression :" "%.4f" % accu)
...
def method_Naive_bayes(x_train,x_test):
    ...
    accu = roc_auc_score(y_test,p)
    accu = accu*100
    print("Accuracy on BBOW with Naive bayes calssifier : " ,"%.4f" % accu)
...
def method_feed_forward(x_train,x_test):
    ...
    accu = roc_auc_score(y_test,p)
    accu = accu*100
    print("Accuracy on BBOW with feed forward Neural : " "%.4f" % accu)
...
def method_SVM(x_train,x_test):
    ...
    accu = roc_auc_score(y_test,p)
    accu = accu*100
    print("Accuracy on BBOW with SVM calssifier : " "%.4f" % accu)
...
training_direc = 'F:\\ACADS\\acads 6th sem\\NLP\\Assignments\\Assign 2\\train_BBOW.txt'
testing_direc = 'F:\\ACADS\\acads 6th sem\\NLP\\Assignments\\Assign 2\\test_BBOW.txt'
